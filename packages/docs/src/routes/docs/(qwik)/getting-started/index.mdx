...
To play with it right away, check out our in-browser playgrounds: hello
...
We've built a basic application as an overview of the key Qwik concepts and its API. The application is running in dev mode, which uses hot-module-reloading (HMR) to continuously update the application while changing the code.
