...
- In Angular, it's called Content Projection
- In React, it's the `children` of the props
- In Web components it's [`<slot>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/slot)
...
Now, when consuming the `<Tab>` component, you can pass children and specify in which slot they should be placed, using the `q:slot` attribute:
...
> **Note**: Make sure to use `<Slot />` within a `component$()` function to ensure it works correctly. `<Slot />` cannot operate in [inline components](/docs/components/overview/#inline-components) which are similar to normal functions `export const MyInlineComp = () => `.
